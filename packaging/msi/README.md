...
```powershell
msiexec /i MaintenanceToolkit-4.0.2-x64.msi CREATE_TASK=1 TASK_TIME=03:00 INVENTORY_SHARE="\\SERVER\MT"
```

Silent standalone example:

```powershell
msiexec /i MaintenanceToolkit-4.0.2-x64.msi /qn CREATE_TASK=1 TASK_TIME=03:00 INVENTORY_SHARE="\\SERVER\MT"
```
...
```powershell
msiexec /i MaintenanceToolkit-4.0.2-x64.msi /qn CREATE_TASK=0
```
